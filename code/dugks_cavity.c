...
void boundary()
{
    ...
    for(i = imin - 1; i <= imax + 1; i++)
    {
        ...
        for(ky = 0; ky < Qy; ky++)
        {
            if(ey[ky] < 0)
                for(kx = 0; kx < Qx; kx++)
                    f_plus[jmax + 1][i][kx][ky] = f_plus[jmax + 1][i][re[kx]][re[ky]] + f_plus[jmax][i][re[kx]][re[ky]] - f_plus[jmax][i][kx][ky] + 4 * rho_b * tpx[kx] * tpy[ky] * (ex[kx] * u_wall) / RT;
        }
    }
}
    ...
    for(i = imin; i <= imax + 1; i++) // inner nodes
    {
	    ...
	    for(ky = 0; ky < Qy;ky++)
            for(kx = 0; kx < Qx; kx++)
            {
                ...
                dfy = 0.5 * (AC * (f_plus[j][iL][kx][ky] + f_plus[j][i][kx][ky]) + AR * (f_plus[jR][iL][kx][ky] + f_plus[jR][i][kx][ky]) - AL * (f_plus[jL][iL][kx][ky] + f_plus[jL][i][kx][ky]));
                x = 0.5 * ex[kx] * dt;
    ...
    for(j = jmin; j <= jmax + 1; j++)
    {
        ...
        for(ky = 0; ky < Qy; ky++)
            for(kx = 0; kx < Qx; kx++)
            {
                ...
                dfx = 0.5 * (AC * (f_plus[jL][i][kx][ky] + f_plus[j][i][kx][ky]) + AR * (f_plus[jL][iR][kx][ky] + f_plus[j][iR][kx][ky]) - AL * (f_plus[jL][iL][kx][ky] + f_plus[j][iL][kx][ky]));
                y = 0.5 * ey[ky] * dt;
    ...
    for(j = jmin; j <= jmax; j++)
    {
        ...
        for(i = imin; i <= imax; i++)
        {
            ...
            for(kx = 0; kx < Qx; kx++)
                for(ky = 0; ky < Qy; ky++)
                {
                    f[j][i][kx][ky] = (4.0 * f_plus[j][i][kx][ky] - f[j][i][kx][ky]) / 3.0 + ex[kx] * dt / dx * (xf_face[i][kx][ky] - xf_face[i + 1][kx][ky]);
                }
        }
    }
    ...
    for(i = imin; i <= imax; i++)
    {
        ...
        for(j = jmin; j <= jmax; j++)
        {
            ...
            for(kx = 0; kx < Qx; kx++)
                for(ky = 0; ky < Qy; ky++)
                {
                    f[j][i][kx][ky] = f[j][i][kx][ky] + ey[ky] * dt / dy * (yf_face[j][kx][ky] - yf_face[j +  1][kx][ky]);
                }
        }
    }
...
void  datadeal(int t)
{
    ...
    sprintf( szFileName, "%s.%i.dat","cavity", t );
    ...
    fprintf(fp, "ZONE T=\"BOX\",I=");
    fprintf(fp, "%i", imax - imin + 1);
    fprintf(fp, ",J=");
    fprintf(fp, "%i", jmax - jmin + 1);
    fprintf(fp, ",F = POINT");
    fprintf(fp,"\n");
    ...
    for(y = jmin; y <= jmax; y++)
    {
        for(x = imin; x <= imax; x++)
        {
            ...
            fprintf(fp, "%f %f %f %f %f\n", xc[x], yc[y], rho[y][x], ux[y][x], uy[y][x]);
        }
    }
