...
\caption{}
\end{subfigure}
\caption{Transient isotherms on the centre plane $X=0.5$ for the baseline cubical enclosure. The case uses $\tau_L=1.0$, $\omega=0$, $\varepsilon=1.0$, and $N_{cr}=0.01$. The retained times are $t^*=0.005$, $0.015$, $0.050$, and steady state.}
\label{fig8}
\end{figure}

Figure~\ref{fig8} follows the baseline isotherm sequence reported by Sun et al.~\cite{SunMaLi2012}. The hot south wall first produces a steep thermal layer near $Y=0$, and the high-temperature region then penetrates towards the enclosure interior. The contours remain symmetric about the mid-plane in $Z$, which is consistent with the identical bottom and top wall temperatures. The last transient field is already close to equilibrium, but the steady-state contours retain a slightly sharper central lobe; over the plotted centre plane, the maximum absolute temperature difference between $t^*=0.050$ and steady state is $2.1\times10^{-3}$. The centre point $(X,Y,Z)=(0.5,0.5,0.5)$ warmed from $\theta=0.542$ at $t^*=0.005$ to $\theta=0.668$ at steady state.
